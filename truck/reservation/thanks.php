...
$reserve_data = getReservation($reservation_id);
$reserve_data = getReservatinData($reserve_data['place']);
$left_seat = $reserve_data['count'];

$entry_data = getEntry($reservation_id);
foreach ($entry_data as $entry) {
  if ($entry['status'] != 2) {
    $left_seat -= $entry['count'];
  }
}
...
if (empty($_SESSION['flg'])) {
  ...
  $search = array('{{会社名}}', '{{予約名}}', '{{開始日}}', '{{開始曜日}}', '{{終了日}}', '{{終了曜日}}', '{{予約人数}}');
...
<head>
  ...
  <meta name="title" content="初任運転者講習受講予約｜トラックドライバー教育のクラウド型eラーニング【グッドラーニング!】" />
  <meta name="description" content="グッドラーニング！についての初任運転者講習受講予約はこちらから受け付けております。お気軽にお問合せください。" />
  <meta name="keywords" content="乗務員,教育,講習,Eラーニング,指導,国交省,トラック,運送業,グッドラーニング" />
  <meta property="og:title" content="初任運転者講習受講予約｜トラックドライバー教育のクラウド型eラーニング【グッドラーニング!】" />
  ...
  <meta property="og:description" content="初任運転者講習受講予約はこちらから受け付けております。お気軽にお問合せください。" />
    ...
    <main>
        ...
        <div id="breadCrumbs">
          <ol itemscope="" itemtype="https://schema.org/BreadcrumbList">
            <li itemprop="itemListElement" itemscope="" itemtype="https://schema.org/ListItem">
              <a href="/truck/price/" itemprop="item"><span itemprop="name">HOME</span></a>
      ...
      <div class="inner">
        <div id="contact" class="thanks">
          <h2>初任運転者講習受講予約を承りました。</h2>
          <p class="thanksTxt">
            この度は「初任運転者講習受講予約」をいただき、誠にありがとうございます。<br />
            <br />
            <b>折り返し、弊社担当より連絡申し上げます。今しばらくお待ちくださいませ。</b><br />
            <br />
            なお、入力いただいたメールアドレス宛に受付完了メールを配信致しました。<br />
            しばらくの時間が経過しても受付完了メールが届かない場合、処理が正常に行われていない可能性、メールアドレスの相違などが考えられます。<br /><br />大変お手数ではございますが、弊社まで連絡を頂戴できれば幸いです。<br />電話：03-6880-1064（平日 9:30〜17:00）
          </p>
        </div>
      </div>
    </main>
