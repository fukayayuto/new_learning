...
if (!empty($_POST)) {
  ...
  foreach ($check_list as $k => $val) {
    ...
    $tmp['company_name'] = $accountData[0]['company_name'];
  ...
  if (!empty($_GET['account'])) {
    $account_list = $_GET['account'];
    $account_lisy_array = explode('¥', $account_list);

    foreach ($account_lisy_array as $k => $val) {
      $tmp = array();

      $account_data = getAccount($val);

      $tmp['id'] = $account_data[0]['id'];
      $tmp['company_name'] = $account_data[0]['company_name'];
      $tmp['email'] = $account_data[0]['email'];
